...
function sendCsgoCardsBulk(sender, users, cards, sessionid){
  ...
  for(var i=0;0<min;i++){
    ...
    if(i in users){
      ...
      if(min<Math.max(0, parseInt(8-(users[i].level+Math.max(2-users[i].com)+users[i].csgo+1))) && min > 0){
        i++;
      }
      min = min-Math.max(0, parseInt(8-(users[i].level+Math.max(2-users[i].com)+users[i].csgo+1)));
      cnt = i+1;
    } else {
      break;
    }
  ...
  (function next(counter, maxLoops){
    ...
    var cdata = [];
    var len = Math.min.apply(Math,[(cmin-ccnt),Math.max(0, parseInt(8-(users[counter-1].level+Math.max(2-users[counter-1].com)+users[counter-1].csgo+1)))]);
